...
const WellheadM17Chart = () => {
    ...
    const GetData = async () => {
        const data = await useGetApi('pad_a/wellhead/m1/chart');
        if (data.error === false) {
            console.log(data.data.values)
            let yData1 = data.data.values.data.map(i => i.pshH_3001)
            let yData2 = data.data.values.data.map(i => i.pT_3001)
...
export default WellheadM17Chart