import React from 'react';
import Horizontal from '../pipe/Horizontal';
import pl from '../../../assets/img/faceplate/pl.png'
import gauge from '../../../assets/img/faceplate/gauge.png'
import ph from '../../../assets/img/faceplate/pipe_header.png'
import plank from '../../../assets/img/faceplate/plank.png'
import meter from '../../../assets/img/faceplate/meter.png'
import whrun from '../../../assets/img/faceplate/wellhead-run.png'
import coriolis from '../../../assets/img/faceplate/coriolis.png'
import './waterDisposal.css'
import Vertical from '../pipe/Vertical';

const WaterDisposalSatu = (props) => {
    return (
        <div className='card large'>
            <div className='card-header text-center fw-semibold'>Water Disposal</div>
            <div className='card-body bg-dark'>
                <div className='wtr-dsp-wrapper'>
                    <div className='wtr-dsp-from-pump top'>
                        <span className='wtr-dsp-from-pump'>From Disposal Pump</span>
                        <span className='wtr-dsp-tag'>PP-510 FROM CPA</span>
                    </div>
                    <div className='wtr-dsp-from-pump bottom'>
                        <span className='wtr-dsp-from-pump'>From Disposal Pump</span>
                        <span className='wtr-dsp-tag'>PP-510 FROM CPA</span>
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-1'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-2'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-3'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-4'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-5'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-6'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-7'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-8'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-9'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-10'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-11'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-12'>
                        <Horizontal />
                    </div>

                    <img src={pl} alt="pigLauncher" className='wtr-dsp-pl-1' />
                    <img src={pl} alt="pigLauncher" className='wtr-dsp-pl-2' />
                    <img src={pl} alt="pigLauncher" className='wtr-dsp-pl-3' />
                    <img src={ph} alt="pigLauncher" className='wtr-dsp-pl-4' />
                    <div className='wtr-dsp-pipe-vertical-1'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-2'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-3'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-4'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-5'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-6'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-7'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-8'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-9'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-10'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-11'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-12'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-13'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-14'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-15'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-16'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-17'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-18'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-19'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-20'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-21'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-22'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-23'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-24'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-25'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-26'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-27'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-28'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-29'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-30'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-31'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-32'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-33'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-34'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-35'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-36'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-37'>
                        <Vertical />
                    </div>
                    <div className='wtr-dsp-pipe-vertical-38'>
                        <Vertical />
                    </div>
                    {/* <div className='wtr-dsp-pipe-vertical-39'>
                        <Vertical />
                    </div> */}
                    <div className='wtr-dsp-pipe-vertical-40'>
                        <Vertical />
                    </div>
                    {/* <div className='wtr-dsp-pipe-vertical-41'>
                        <Vertical />
                    </div> */}
                    {/* <div className='wtr-dsp-pipe-vertical-42'>
                        <Vertical />
                    </div> */}
                    {/* <div className='wtr-dsp-pipe-vertical-43'>
                        <Vertical />
                    </div> */}


                    <div className='wtr-dsp-pipe-horizontal-13'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-14'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-15'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-16'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-17'>
                        <Horizontal />
                    </div>
                    <div className='wtr-dsp-pipe-horizontal-18'>
                        <Horizontal />
                    </div>


                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-1' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-2' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-3' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-4' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-5' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-6' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-7' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-8' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-9' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-10' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-11' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-12' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-13' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-14' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-15' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-16' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-17' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-18' />
                    <img src={coriolis} alt="" className='wtr-dsp-coriolis-19' />
                    {/* <img src={coriolis} alt="" className='wtr-dsp-coriolis-20' /> */}
                    <img src={plank} alt="" className='wtr-dsp-plank-1' />
                    <img src={plank} alt="" className='wtr-dsp-plank-2' />
                    <img src={plank} alt="" className='wtr-dsp-plank-3' />
                    <img src={plank} alt="" className='wtr-dsp-plank-4' />
                    <img src={plank} alt="" className='wtr-dsp-plank-5' />
                    <img src={plank} alt="" className='wtr-dsp-plank-6' />
                    <img src={plank} alt="" className='wtr-dsp-plank-7' />
                    <img src={plank} alt="" className='wtr-dsp-plank-8' />
                    <img src={plank} alt="" className='wtr-dsp-plank-9' />
                    {/* <img src={plank} alt="" className='wtr-dsp-plank-10' /> */}
                    {/* <img src={plank} alt="" className='wtr-dsp-plank-11' /> */}
                    <img src={meter} alt="" className='wtr-dsp-meter-1' />
                    <img src={meter} alt="" className='wtr-dsp-meter-2' />
                    <img src={meter} alt="" className='wtr-dsp-meter-3' />
                    {/* <img src={meter} alt="" className='wtr-dsp-meter-4' /> */}
                    <img src={whrun} alt="" className='wtr-dsp-wellhead-1' />
                    <img src={whrun} alt="" className='wtr-dsp-wellhead-2' />
                    <img src={whrun} alt="" className='wtr-dsp-wellhead-3' />
                    <img src={whrun} alt="" className='wtr-dsp-wellhead-4' />
                    <div className='wtr-dsp-label-1'>
                        <span className='span-text'>PL-2000</span>
                    </div>
                    <div className='wtr-dsp-label-2'>
                        <span className='span-text'>PL-2009</span>
                    </div>
                    <div className='wtr-dsp-label-3'>
                        <span className='span-text'>PR-2020</span>
                    </div>
                    <div className='wtr-dsp-label-4'>
                        <span className='span-text'>PIPE HEADER RECEIVER</span>
                    </div>
                    <div className='wtr-dsp-label-5'>
                        <span className='label-tag'>Wellhead M6</span>
                    </div>
                    <div className='wtr-dsp-label-6'>
                        <span className='label-tag'>Wellhead M8</span>
                    </div>
                    <div className='wtr-dsp-label-7'>
                        <span className='label-tag'>Wellhead M12</span>
                    </div>
                    <div className='wtr-dsp-label-8'>
                        <span className='label-tag'>Wellhead M14</span>
                    </div>

                    <div className='wtr-dsp-label-9'>
                        <span className='label-tag'>FUTURE CONNECTION</span>
                    </div>
                    <div className='wtr-dsp-label-10'>
                        <span className='label-tag'>PT</span>
                    </div>
                    <div className='wtr-dsp-label-11'>
                        <span className='label-tag'>TT</span>
                    </div>

                    <img src={gauge} alt="" className='wtr-dsp-pt-1' />
                    <img src={gauge} alt="" className='wtr-dsp-pt-2' />

                    <div className="d-flex flex-column bg-light p-1 border border-primary wtr-dsp-monitor-1">
                        <span className="p-1 text-center bg-dark fw-bolder text-white wtr-dsp-monitor-tag">
                            {props.tag1}
                        </span>
                        <span className="p-1 text-center fw-bold wtr-dsp-monitor-value">
                            {props.tag1Status ? props.tag1Status : "####"}  bw/d
                        </span>
                    </div>
                    <div className="d-flex flex-column bg-light p-1 border border-primary wtr-dsp-monitor-2">
                        <span className="p-1 text-center bg-dark fw-bolder text-white wtr-dsp-monitor-tag">
                            {props.tag2}
                        </span>
                        <span className="p-1 text-center fw-bold wtr-dsp-monitor-value">
                            {props.tag2Status ? props.tag2Status : "####"}  bw/d
                        </span>
                    </div>
                    <div className="d-flex flex-column bg-light p-1 border border-primary wtr-dsp-monitor-3">
                        <span className="p-1 text-center bg-dark fw-bolder text-white wtr-dsp-monitor-tag">
                            {props.tag3}
                        </span>
                        <span className="p-1 text-center fw-bold wtr-dsp-monitor-value">
                            {props.tag3Status ? props.tag3Status : "####"}  bw/d
                        </span>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default WaterDisposalSatu