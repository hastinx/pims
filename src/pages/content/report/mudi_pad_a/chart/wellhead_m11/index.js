...
const WellheadM11Chart = () => {
    ...
    const GetData = async () => {
        const data = await useGetApi('pad_a/wellhead/m11/chart');
        if (data.error === false) {
            console.log(data.data.values)
            let yData1 = data.data.values.data.map(i => i.pshH_3011)
            let yData2 = data.data.values.data.map(i => i.pT_3011)
...
export default WellheadM11Chart