...
const WellheadM21Chart = () => {
    ...
    const GetData = async () => {
        const data = await useGetApi('pad_a/wellhead/m21/chart');
        if (data.error === false) {
            console.log(data.data.values)
            let yData1 = data.data.values.data.map(i => i.pshH_3021)
            let yData2 = data.data.values.data.map(i => i.pT_3021)
...
export default WellheadM21Chart