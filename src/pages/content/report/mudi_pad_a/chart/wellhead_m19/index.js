...
const WellheadM19Chart = () => {
    ...
    const GetData = async () => {
        const data = await useGetApi('pad_a/wellhead/m19/chart');
        if (data.error === false) {
            console.log(data.data.values)
            let yData1 = data.data.values.data.map(i => i.pshH_3019)
            let yData2 = data.data.values.data.map(i => i.pT_3019)
...
export default WellheadM19Chart