...
const WellheadM22Chart = () => {
    ...
    const GetData = async () => {
        const data = await useGetApi('pad_a/wellhead/m22/chart');
        if (data.error === false) {
            console.log(data.data.values)
            let yData1 = data.data.values.data.map(i => i.pshH_3022)
            let yData2 = data.data.values.data.map(i => i.pT_3022)
...
export default WellheadM22Chart